...
int main()
{
    ...
    while (t--)
    {
        ...
        forsn(i, 1, n - 1)
        {
            if (a[i] - a[i + 1] <= prev)
            {
                // print(a[i] - a[i + 1], prev);
                cost_rev.pb(cost_rev.back() + 1);
                prev = a[i] - a[i + 1];
            }
            else
            {
                cost_rev.pb(cost_rev.back() + a[i] - a[i + 1]);
                prev = a[i] - a[i + 1];
            }
        }
        reverse(cost_rev.begin(), cost_rev.end());
        ll k;
        cin >> k;
        forn(i, k)
        {
            ...
            if (l > r)
            {
                print(cost_rev[r - 1] - cost_rev[l - 1]);
            }
            else if (l < r)
            {
